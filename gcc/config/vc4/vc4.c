...
static void mcore_asm_named_section(const char *, unsigned int, tree);
#endif
...
static void mcore_print_operand_address(FILE * stream, rtx x)
{
    switch (GET_CODE(x))
    {
        case REG:
 ...
 * to modifier code.
 * 
 * 'R' print the next register or memory location along, i.e. the lsw in a 
 * double word value 'C' print a constant with the # 'O' print a constant
 * without the # 'M' print a constant as its negative 'P' print log2 of a
 * power of two 'Q' print log2 of an inverse of a power of two 'U' print
 * register for ldm/stm instruction 'X' print byte number for xtrbN
 * instruction.  
 */
    ...
    switch (code)
    {
        ...
        case 'P':
            fprintf(asm_out_file, "%d",
                    exact_log2(INTVAL(x) & 0xffffffff));
            break;
        case 'Q':
...
bool mcore_gen_compare(enum rtx_code code, rtx op0, rtx op1)
{
    ...
    if (GET_CODE(op1) == CONST_INT)
    {
        ...
        switch (code)
        {
            case GTU:
                /*
                 * Unsigned > 0 is the same as != 0; everything else is
                ...
                /*
                 * Check whether (LE A imm) can become (LT A imm + 1), or (GT
                 * A imm) can become (GE A imm + 1).  
                 */
            case GT:
            case LE:
    ...
    /*
     * cmpnei: 0-31 (K immediate) cmplti: 1-32 (J immediate, 0 using btsti 
     * x,31).  
     */
    invert = false;
    switch (code)
    ...
    emit_insn(gen_rtx_SET(VOIDmode,
                          cc_reg, gen_rtx_fmt_ee(code, CCmode, op0, op1)));
    return invert;
}

int mcore_symbolic_address_p(rtx x)
{
    switch (GET_CODE(x))
    {
        case SYMBOL_REF:
...
char *mcore_output_call(rtx operands[], int index)
{
...
int const_ok_for_mcore(HOST_WIDE_INT value)
{
...
int mcore_const_ok_for_inline(HOST_WIDE_INT value)
{
    HOST_WIDE_INT x, y;
...
int mcore_const_trick_uses_not(HOST_WIDE_INT value)
{
    HOST_WIDE_INT x, y;
 ...
 * 
 * 0: not inlinable 1: single instruction (do the usual thing) 2: single
 * insn followed by a 'not' 3: single insn followed by a subi 4: single
 * insn followed by an addi 5: single insn followed by rsubi 6: single
 * insn followed by bseti 7: single insn followed by bclri 8: single insn
 * followed by rotli 9: single insn followed by lsli 10: single insn
 * followed by ixh 11: single insn followed by ixw.  
 */
...
int mcore_is_dead(rtx first, rtx reg)
{
    rtx insn;
    ...
    /*
     * Look for conclusive evidence of live/death, otherwise we have to
     * assume that it is live.  
     */
    ...
    /*
     * No conclusive evidence either way, we cannot take the chance that
     * control flow hid the use from us -- "I'm not dead yet".  
     */
    return 0;
}
...
int mcore_num_ones(HOST_WIDE_INT mask)
{
    /*
...
int mcore_num_zeros(HOST_WIDE_INT mask)
{
...
int mcore_byte_offset(unsigned int mask)
{
    if (mask == 0x00ffffffL)
        return 0;
...
int mcore_halfword_offset(unsigned int mask)
{
    if (mask == 0x0000ffffL)
        return 0;
...
const char *mcore_output_bseti(rtx dst, int mask)
{
    rtx out_operands[2];
    int bit;
...
const char *mcore_output_bclri(rtx dst, int mask)
{
    rtx out_operands[2];
    int bit;
...
const char *mcore_output_cmov(rtx operands[], int cmp_t, const char *test)
{
...
const char *mcore_output_andn(rtx insn ATTRIBUTE_UNUSED, rtx operands[])
{
    HOST_WIDE_INT x, y;
    rtx out_operands[3];
    const char *load_op;
...
static const char *output_inline_const(enum machine_mode mode,
                                       rtx operands[])
{
     ...
     * turned into lrw's.  Our caller uses try_constant_tricks to back off 
     * to an lrw rather than calling this routine.  
     */
    ...
    switch (trick_no)
    {
        case 1:
            ...
            break;
        case 2:                /* not */
            sprintf(buf, "%s\n\tnot\t%s\t// %ld 0x%lx", load_op, dst_fmt,
                    value, value);
            break;
        case 3:                /* add */
...
const char *mcore_output_move(rtx insn ATTRIBUTE_UNUSED, rtx operands[],
    ...
    if (GET_CODE(dst) == REG)
    {
        ...
        else if (GET_CODE(src) == CONST_INT)
        {
            ...
            if (CONST_OK_FOR_I(INTVAL(src)))    /* r-I */
                ...
                return output_inline_const(SImode, operands);   /* 1-2
                                                                 * insns */
            else
                return "lrw\t%0,%x1\t// %1";    /* Get it from literal
                                                 * pool.  */
        }
        else
...
/*
 * Return a sequence of instructions to perform DI or DF move. Since the
 * MCORE cannot move a DI or DF in one instruction, we have to take care
 * when we see overlapping source and dest registers.  
 */

const char *mcore_output_movedouble(rtx operands[],
                                    enum machine_mode mode
                                    ATTRIBUTE_UNUSED)
{
    rtx dst = operands[0];
    ...
    if (GET_CODE(dst) == REG)
    {
        ...
        else if (GET_CODE(src) == MEM)
        {
            ...
            if (dstreg == basereg)
            {
                ...
                /*
                 * XXX: alternative: move basereg to basereg+1 and then
                 * fall through.  
                 */
            }
            else
...
int mcore_arith_S_operand(rtx op)
{
...
int mcore_expand_insv(rtx operands[])
{
     ...
     * 191) for width==1 must be removed.  Look around line 368.  This is
     * something we really want the md part to do.  
     */
...

/*
 * ??? Block move stuff stolen from m88k.  This code has not been verified 
 * for correctness.  
 */
 ...
 * 
 * OPERANDS[0] is the destination. OPERANDS[1] is the source. OPERANDS[2]
 * is the size. OPERANDS[3] is the alignment safe to use.  
 */

/*
 * Emit code to perform a block move with an offset sequence of ldw/st
 * instructions (..., ldw 0, stw 1, ldw 1, stw 0, ...).  SIZE and ALIGN
 * are known constants.  DEST and SRC are registers.  OFFSET is the known
...
bool mcore_expand_block_move(rtx * operands)
{
                           ...
                           int second_time ATTRIBUTE_UNUSED)
{
    ...
    number_of_regs_before_varargs =
        *args_so_far + mcore_num_arg_regs(mode, type);
...
/*
 * The MCORE cannot load a large constant into a register, constants have
 * to come from a pc relative load.  The reference of a pc relative load
 ...
 * 
 * lrw L1,r0 br L2 align L1: .long value L2: ..
 * 
 * lrw L3,r0 br L4 align L3: .long value L4: ..
 * 
 ...
 * The algorithm is:
 * 
 * scan, find an instruction which needs a pcrel move.  Look forward, find
 * the last barrier which is within MAX_COUNT bytes of the requirement. If
 * there isn't one, make one.  Process all the instructions between the find 
 * and the barrier.
 * 
 * In the above example, we can tell that L3 is within 1k of L1, so the
 * first move can be shrunk from the 2 insn+constant sequence into just 1
 * insn, and the constant moved to L3 to make:
 * 
 * lrw L1,r0 .. lrw L3,r0 bra L4 align L3:.long value L4:.long value
 * 
...
/*
 * The maximum number of constants that can fit into one pool, since the
 * pc relative range is 0...1020 bytes and constants are at least 4 bytes
 * long.  We subtract 4 from the range to allow for the case where we need 
 * to add a branch/align before the constant pool.  
 */
...
/*
 * Dump out any constants accumulated in the final pass.  These will only
 * be labels.  
 */

const char *mcore_output_jump_label_table(void)
{
    int i;
...
static cond_type is_cond_candidate(rtx insn)
{
    /*
     * The only things we conditionalize are those that can be directly
     * changed into a conditional.  Only bother with SImode items.  If we 
     * wanted to be a little more aggressive, we could also do other modes 
     * such as DImode with reg-reg move or load 0.  
     */
    if (GET_CODE(insn) == INSN)
    {
        ...
        if ((GET_CODE(dst) != REG &&
             GET_CODE(dst) != SUBREG) || GET_MODE(dst) != SImode)
            return COND_NO;

        src = XEXP(pat, 1);

        if ((GET_CODE(src) == REG ||
             (GET_CODE(src) == SUBREG && GET_CODE(SUBREG_REG(src)) == REG))
            && GET_MODE(src) == SImode)
        ...
        /*
         * Some insns that we don't bother with: (set (rx:DI) (ry:DI))
...
static rtx emit_new_cond_insn(rtx insn, int cond)
{
    rtx c_insn = 0;
    rtx pat, dst, src;
    cond_type num;
     ...
     */
    if (rtx_length[GET_CODE(c_insn)] >= 7
        && rtx_length[GET_CODE(insn)] >= 7)
    {
        /*
         * We really don't need to bother with the notes and links at this
         * point, but go ahead and save the notes.  This will help
         * is_dead() when applying peepholes (links don't matter since they 
         * are not used any more beyond this point for the mcore).  
         */
    ...
    {
        /*
         * For jumps, we need to be a little bit careful and emit the new
         * jump before the old one and to update the use count for the
         * target label. This way, the barrier following the old (uncond)
         * jump will get deleted, but the label won't.  
         */
...
/*
 * Attempt to change a basic block into a series of conditional insns.
 * This works by taking the branch at the end of the 1st block and
 * scanning for the end of the 2nd block.  If all instructions in the 2nd 
 * block have cond. versions and the label at the start of block 3 is the
 * same as the target from the branch at block 1, then conditionalize all
 * insn in block 2 using the inverse condition of the branch at block 1.
 * (Note I'm bending the definition of basic block here.)
 * 
 * e.g., change:
 * 
 * bt L2 <-- end of block 1 (delete) mov r7,r8 addu r7,1 br L3 <-- end
 * of block 2
 * 
 * L2: ...  <-- start of block 3 (NUSES==1) L3: ...
 * 
 * to:
 * 
 * movf r7,r8 incf r7 bf L3
 * 
 * L3: ...
 * 
 ...
 */
static rtx conditionalize_block(rtx first)
{
    rtx insn;
    rtx br_pat;
    rtx end_blk_1_br = 0;
    ...
    /*
     * Complement the condition since we use the reverse cond. for the
     * insns.  
     */
    ...
    {
        /*
         * An inverse branch, so extract the label out of the 2nd arm and
    ...
    /*
     * Scan forward for the start of block 2: it must start with a label
     * and that label must be the same as the branch target label from
     * block 1.  We don't care about whether block 2 actually ends with a
     * branch or a label (an uncond. branch is conditionalizable).  
     */
    for (insn = NEXT_INSN(first); insn; insn = NEXT_INSN(insn))
    {
        enum rtx_code code;
        ...
        /*
         * Skip barriers, notes, and conditionalizable insns.  If the insn 
         * is not conditionalizable or makes this optimization fail, just
         * return the next insn so we can start over from that point.  
         */
        if (code != BARRIER && code != NOTE && !is_cond_candidate(insn))
            return NEXT_INSN(insn);

        /*
         * Remember the last real insn before the label (i.e. end of block 
         * 2).  
         */
    ...
    /*
     * It is possible for this optimization to slow performance if the
     * blocks are long.  This really depends upon whether the branch is
     * likely taken or not.  If the branch is taken, we slow performance
     * in many cases.  But, if the branch is not taken, we always help
     * performance (for a single block, but for a double block (i.e. when 
     * the optimization is re-applied) this is not true since the 'right
     * thing' depends on the overall length of the collapsed block).  As a 
     * compromise, don't apply this optimization on blocks larger than
     * size 2 (unlikely for the mcore) when speed is important. the best
     * threshold depends on the latencies of the instructions (i.e., the
     * branch penalty).  
     */
    ...
    /*
     * At this point, we've found the start of block 3 and we know that it 
     * is the destination of the branch from block 1.  Also, all
         ...
         insn = NEXT_INSN(insn))
    {
        ...
        /*
         * Try to form a conditional variant of the instruction and emit
         * it.  
         */
    ...
    /*
     * ??? we probably should redistribute the death notes for this insn,
     * esp. the death of cc, but it doesn't really matter this late in the 
     * game. The peepholes all use is_dead() which will find the correct
     * death regardless of whether there is a note.  
     */
...
/*
 * Apply the conditionalization of blocks optimization.  This is the outer 
 * loop that traverses through the insns scanning for a branch that
 * signifies an opportunity to apply the optimization.  Note that this
 * optimization is applied late.  If we could apply it earlier, say before 
 * cse 2, it may expose more optimization opportunities. but, the pay
 * back probably isn't really worth the effort (we'd have to update all
 * reg/flow/notes/links/etc to make it work - and stick it in before cse
 * 2).  
 */
...
bool mcore_r15_operand_p(rtx x)
{
    switch (GET_CODE(x))
    {
        case CONST_INT:
 ...
 * RCLASS.  If X is too complex to move directly into r15, prefer to use
 * LRW_REGS instead.  
 */

#if 0
enum reg_class mcore_reload_class(rtx x, enum reg_class rclass)
{
...
int mcore_is_same_reg(rtx x, rtx y)
{
    /*
...
int mcore_num_arg_regs(enum machine_mode mode, const_tree type)
{
    int size;
...
handle_structs_in_regs(enum machine_mode mode, const_tree type, int reg)
{
    ...
    /*
     * The MCore ABI defines that a structure whose size is not a whole
    ...
    if (type
        ...
        && (reg + mcore_num_arg_regs(mode, type) <=
...
rtx mcore_function_value(const_tree valtype, const_tree func)
{
    enum machine_mode mode;
    int unsigned_p;
...
int mcore_dllexport_name_p(const char *symbol)
{
...
int mcore_dllimport_name_p(const char *symbol)
{
...
static void mcore_mark_dllexport(tree decl)
{
    const char *oldname;
    char *newname;
    rtx rtlname;
...
static void mcore_mark_dllimport(tree decl)
{
    const char *oldname;
    char *newname;
    tree idp;
    ...
    /*
     * ??? One can well ask why we're making these checks here, and that
    ...
    if (TREE_CODE(decl) == VAR_DECL
        && !DECL_VIRTUAL_P(decl) && DECL_INITIAL(decl))
    {
    ...
    /*
     * `extern' needn't be specified with dllimport. Specify `extern' now
...
static int mcore_dllexport_p(tree decl)
{
...
static int mcore_dllimport_p(tree decl)
{
    if (TREE_CODE(decl) != VAR_DECL && TREE_CODE(decl) != FUNCTION_DECL)
        return 0;
...
static void
mcore_encode_section_info(tree decl, rtx rtl ATTRIBUTE_UNUSED,
                          int first ATTRIBUTE_UNUSED)
{
     ...
     */
             ...
             &&
             mcore_dllimport_name_p(XSTR
                                    (XEXP(XEXP(DECL_RTL(decl), 0), 0), 0)))
    {
        ...
        /*
         * We previously set TREE_PUBLIC and DECL_EXTERNAL. ??? We leave
         * these alone for now.  
         */
    }
}
...
static const char *mcore_strip_name_encoding(const char *str)
{
    return str + (str[0] == '@' ? 3 : 0);
}

/*
 * MCore specific attribute support. dllexport - for exporting a
 * function/variable that will live in a dll dllimport - for importing a
 * function/variable from a dll naked - do not create a function
 * prologue/epilogue.  
 */
...
/*
 * ??? It looks like this is PE specific? Oh well, this is what the old
 * code did as well.  
 */

static void mcore_unique_section(tree decl, int reloc ATTRIBUTE_UNUSED)
{
    int len;
    const char *name;
    char *string;
    ...
    /*
     * The object is put in, for example, section .text$foo. The linker
     * will then ultimately place them in .text (everything from the $ on
     * is stripped).  
     */
    if (TREE_CODE(decl) == FUNCTION_DECL)
        prefix = ".text$";
    /*
     * For compatibility with EPOC, we ignore the fact that the section
     * might have relocs against it.  
     */
...
int mcore_naked_function_p(void)
{
...
static bool mcore_warn_func_return(tree decl)
{
    /*
...
static void mcore_external_libcall(rtx fun)
{
...
/*
 * Worker function for TARGET_ASM_TRAMPOLINE_TEMPLATE. Output assembler
 * code for a block containing the constant parts of a trampoline, leaving 
 * space for the variable parts.
 * 
 * On the MCore, the trampoline looks like: lrw r1, function lrw r13, area
 * jmp r13 or r0, r0 .literals 
 */

static void mcore_asm_trampoline_template(FILE * f)
{
